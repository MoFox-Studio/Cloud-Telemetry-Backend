...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    // Since container has pointer-events: none, these might not trigger natively unless we bind to window or remove it.
    // However, I'll bind them to window but constrain coords so rings interact globally with the dashboard area!
    const globalOnMouseMove = (e) => {
      const rect = mount.getBoundingClientRect();
      mouseRef.current[0] = (e.clientX - rect.left) / rect.width - 0.5;
      mouseRef.current[1] = -((e.clientY - rect.top) / rect.height - 0.5);
    };

    window.addEventListener('mousemove', globalOnMouseMove);
    ...
    return () => {
      ...
      window.removeEventListener('mousemove', globalOnMouseMove);
      ro.disconnect();
