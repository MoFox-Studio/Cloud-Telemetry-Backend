...
void main() {
  ...
  vec2 drift = vec2(
    sin(time * waveSpeed * 0.32 + uv.y * 1.7),
    cos(time * waveSpeed * 0.27 + uv.x * 1.3)
  ) * driftStrength;
  float f = pattern(uv + drift);
  ...
  float intensity = (0.095 + smoothstep(0.05, 0.9, f) * 0.9) * brightness;
  intensity = clamp(intensity, 0.0, 0.82);
  vec3 col = mix(vec3(0.0), waveColor, intensity);
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef({
    ...
    mousePos: new THREE.Uniform(new THREE.Vector2(1, 1)),
    enableMouseInteraction: new THREE.Uniform(enableMouseInteraction ? 1 : 0),
    mouseRadius: new THREE.Uniform(mouseRadius),
    brightness: new THREE.Uniform(brightness),
    driftStrength: new THREE.Uniform(driftStrength)
  });
  ...
  useFrame(({ clock }) => {
    ...
    if (u.brightness.value !== brightness) u.brightness.value = brightness;
    if (u.driftStrength.value !== driftStrength) u.driftStrength.value = driftStrength;
  ...
  return (
    <>
      <mesh ref={mesh} scale={[viewport.width * 1.18, viewport.height * 1.18, 1]}>
...
export default function Dither({
  ...
  waveFrequency = 3,
  waveAmplitude = 0.3,
  waveColor = [0.1, 0.2, 0.4], // Darker bluish tone for the background
  colorNum = 5,
  pixelSize = 2,
